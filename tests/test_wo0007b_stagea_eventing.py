...
def _order(status, order_id="o1", filled=0, broker_order_id=None):
    return Order(
        id=order_id, candidate_id="c1", symbol="AAPL", side=OrderSide.BUY,
        order_type=OrderType.LIMIT, quantity=10, limit_price=1.0,
        status=status, filled_quantity=filled, broker_order_id=broker_order_id,
    )
...
def test_helper_cancel_pending_entry_emits_event_engine_local():
    ev = execution_event_for_routine_transition(
        _order(OrderStatus.SUBMITTED, broker_order_id="brk-1"), OrderStatus.CANCEL_PENDING, 0
    )
    assert ev is not None
...
async def test_store_release_emits_gapless_submit_released(any_store):
    ...
    released = _of(await any_store.get_execution_events(), ExecutionEventType.SUBMIT_RELEASED)
    assert [e.dedupe_key for e in released] == [f"release:{order.id}:0", f"release:{order.id}:1"]
...
async def test_dual_store_stagea_parity(tmp_path):
    ...
    try:
        ...
        def shape(events):
            return [
                (e.event_type.value, e.dedupe_key.split(":", 1)[0], e.source.value, e.authority.value)
                for e in events
                if e.event_type in (ExecutionEventType.SUBMIT_RELEASED, ExecutionEventType.CANCEL_PENDING)
            ]

        assert shape(await memory.get_execution_events()) == shape(await sqlite.get_execution_events())
