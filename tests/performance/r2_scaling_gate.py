"""Deterministic R2 SQLite scaling and query-plan gate.

Run explicitly from the repository root::

    python -m tests.performance.r2_scaling_gate

Set ``R2_STRESS=1`` to add the 1,000-symbol/100,000-event corpus.  This is an
explicit gate rather than an ordinary pytest module: a measured failure is a
request for a separately approved performance/schema work order, not an xfail
hidden inside the correctness suite.
"""
...
def _select_trace(store: SqliteStateStore) -> tuple[list[str], Any]:
    statements: list[str] = []
...
def _plans(connection: Any, statements: list[str]) -> list[dict[str, Any]]:
    plans = []
...
def _unrelated_scans(plans: list[dict[str, Any]]) -> list[str]:
    offenders = []
    for plan in plans:
...
async def _runtime_metrics(store: SqliteStateStore, repetitions: int) -> dict[str, Any]:
...
async def _startup_metrics(path: Path) -> dict[str, Any]:
...
async def _dataset_metrics(root: Path, dataset: Dataset) -> dict[str, Any]:
...
def _projection_peak_bytes() -> int:
    envelopes = []
    actions = []
    orders = {}
    for generation in range(10):
        envelope = _envelope(
            f"projection-envelope-{generation}",
            "projection-owner",
            TARGET,
            active=False,
        )
        ...
        for child in range(10):
            order = _order(
                f"projection-order-{generation}-{child}",
                "projection-owner",
                TARGET,
            )
            ...
            actions.append(
                ExecutionEvent(
                    ...
                    session_id="session-1",
                    correlation_id="projection-owner",
    ...
    for _ in range(20):
        projection = project_envelope_obligation(
            ...
            orders_by_id=orders,
            known_envelopes_by_id={item.id: item for item in envelopes},
        )
...
async def main() -> int:
    with tempfile.TemporaryDirectory(prefix="r2-scaling-") as directory:
        root = Path(directory)
        datasets = {
    ...
    projection_peak_bytes = _projection_peak_bytes()
    gates = {
        ...
        "projection_peak_bytes_le_2_mib": (
            projection_peak_bytes <= PROJECTION_PEAK_LIMIT_BYTES
        ),
    }
    ratios = {
