...
* ``signal_transport_policy`` (A-1): "loopback" | "tailnet_serve", validated.
...
def test_transport_policy_validated(monkeypatch):
    _clear(monkeypatch)
    monkeypatch.setenv("SIGNAL_TRANSPORT_POLICY", "tailnet_serve")
    assert load_settings().signal_transport_policy == "tailnet_serve"
