...
| INV-4 | No oversell, pre-submit and post-fill | A sell whose operator quantity exceeds (live position − outstanding committed sell exposure — where exposure counts every non-terminal LOCAL sell order, `CREATED`/`ORDERED`-pre-submit included, REV-0025-F P1), read inside the A-2 atomic command, is REFUSED (`POSITION_CHANGED`, 05§1/§3a — never silently capped). **Multi-exit is allowed** (Ameen 2026-07-14): concurrent signal exits may coexist, but the combined-exposure ceiling means they can **never jointly oversell**; the pre-submit gate and overfill quarantine run unchanged on each produced order. |
