...
| INV-4 | No oversell, pre-submit and post-fill | A sell whose operator quantity exceeds the live position read under the lock is REFUSED (`POSITION_CHANGED`, 05§1 — never silently capped); the pre-submit gate and overfill quarantine run unchanged on the produced order. |
