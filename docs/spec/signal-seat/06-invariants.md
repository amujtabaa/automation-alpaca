...
| INV-4 | No oversell, pre-submit and post-fill | A sell whose operator quantity exceeds (live position − outstanding committed sell exposure), read inside the A-2 atomic command, is REFUSED (`POSITION_CHANGED`, 05§1/§3a — never silently capped; joint oversell via two signals is structurally excluded); the pre-submit gate and overfill quarantine run unchanged on the produced order. |
