...
parsing before the rate decision and defeat A-4's pre-body defense, archive REV-0024-F-004) →
...
(archive REV-0024-F-002 probe: 10080 events / 7 days at 1/min, bucket never below 9). So each producer also
...
- `signal_invalid_budget_per_epoch: int = 50` — `Settings`-tunable within **`[1, 1000]`**; **1000 is
  ...
  range (mirrors `server_max_ttl`; archive REV-0024-F P2 — the "finite and small" property must not be
  ...
  just-expired proposals (archive REV-0024-F P1). It does **not** refill while the producer is un-quarantined.
- **The debit is linearizable and atomic with the terminal append (archive REV-0025-F-003).** Deciding
...
- **Exact transition on the final slot — the exhausting append opens the epoch in the SAME atomic op**
  (Ameen decision 2026-07-14, archive REV-0025-F P1; this **supersedes** the earlier archive REV-0024 "epoch opens on
...
- **Both the pinned limit AND the consumed/remaining count are durable producer-rail state
  (archive REV-0025-F-004).** It is not enough to persist the cycle's limit: the **consumed count** (or
  ...
  (archive REV-0025-F P1):** the event log alone must reconstruct the binding budget, so **each
...
- **Config changes are cycle-scoped, not retroactive** (archive REV-0024-F P1): a change to
...
indefinitely in an epoch that never quarantines — archive REV-0025-F P2); the constant/flood assertion is
...
## 2. Enablement gated on full rails — the interim ceiling is withdrawn (ADR-009 A-4; archive REV-0024-F-004)
...
archive REV-0024 showed that ceiling was **rate-bounded, not storage-bounded** (a producer paced under the
...
provider — never a scaffold it deletes** (archive REV-0025-F-005). Because a Protocol-presence check cannot
...
archive REV-0025-F-005/F-007). Authored across the WOs, run green at the joint milestone — never against a
...
**and dead-on-arrival `SIGNAL_EXPIRED`** (`expires_at ≤ received_at` / skew-based; archive REV-0024-F P1 —
...
2. Audit is bounded **per quarantine epoch** (epoch = quarantine → release), NOT per time window
   ...
   bucket-only bound is likewise unbounded under paced hostility — archive REV-0024-F-002, which the §1a
