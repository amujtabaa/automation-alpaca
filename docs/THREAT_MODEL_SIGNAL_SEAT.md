...
|---|---|---|
| GAP-01 | ~~R5~~ **SATISFIED** — R5a WO-0137, REV-0041 ACCEPT (addendum) | R5 must land the operator-auth enforcement flip, cockpit `X-Operator-Key` plumbing, and mounted-app proof in the same change, and must demonstrate that kill switch, manual flatten/session controls, and sensitive reads do not become unusable for an operator with the configured key. |
| GAP-02 | ~~R5~~ **SATISFIED** — R5b-2 WO-0139, REV-0043 ACCEPT (addendum) | R5 must fail closed for every mounted route when `signal_seat_enabled=true`: unauthenticated and producer-key callers must receive 401/403 on all sensitive reads/commands, producer keys must authorize only `POST /api/signals`, and the route-introspection test must fail if any mounted route is absent from the matrix, including `POST /api/session/close`. |
| GAP-03 | ~~R5~~ **SATISFIED (launch guard)** — R5a WO-0137/REV-0041; the real-provider contract is the R6 side, see GAP-08 | R5 must keep `signal_seat_enabled` un-enable-able unless the rails-presence contract is satisfied by the R6 provider; a stub, counting-only ceiling, or missing release path must refuse startup rather than expose `POST /api/signals`. |
| GAP-04 | ~~R5~~ **SATISFIED** — R5a WO-0137, REV-0041 | R5 must implement the backend-owned launcher/construction-time bind guard so `loopback` and `tailnet_serve` start only on loopback or same-host socket, and any public/non-loopback bind, Funnel/public reverse proxy mode, bare ASGI app launch, or request-time-503-only guard opens no listener. |
| GAP-05 | ~~R5~~ **SATISFIED** — R5b-1 WO-0138, REV-0042 ACCEPT (addendum) | R5 must treat submitted `thesis` and `provenance` as hostile display text: cockpit/API rendering must not execute HTML/script/unsafe markdown, must preserve verbatim audit content, and must not leak credentials in validation or display errors. |
| GAP-06 | ~~R5~~ **SATISFIED** — R5b-1/R5b-2 WO-0138/WO-0139, REV-0042/REV-0043 | R5 must derive producer identity solely from the authenticated producer key, reject body `producer_id` mismatches before namespace accounting, and prove dedupe, rate-limit, invalid-budget, quarantine, and audit provenance are keyed by the authenticated producer namespace. |
...
| GAP-08 | ~~R6~~ **PARTIALLY SATISFIED** — the store surface landed in R6a (WO-0104a + WO-0140; REV-0044 ACCEPT-WITH-CHANGES, REV-0045 gate OPEN at this writing); provider/route wiring remains R6b | R6 must provide a real dual-rail provider with refilling per-producer bucket, non-refilling durable invalid/conflict/DOA-expiry budget, atomic debit+append, one quarantine opener per epoch, saturating suppressed counter, and human release reset across memory and SQLite. |
...
| GAP-10 | ~~operator NEEDS-INPUT~~ → **CLOSED 2026-07-25** | ~~The operator must decide the signal-sell versus envelope relationship and the multi-exit/single-flight relaxation before R7 implements sell-direction conversion.~~ **Both questions were already ratified when this advisory doc was written; the gap is stale, not open.** (a) *Sell↔envelope:* **ADR-009 D-SIG-8** — "SELL signals mint the same `SellIntent` as cockpit/manual flows… if the operator delegates through an execution envelope, the ordinary ADR-010 path is used; signal correlation is provenance, not authority, and creates no execution lane" (`docs/adr/ADR-009-signal-seat-boundary.md:276-279`). (b) *Multi-exit/single-flight:* **ADR-009 D-SIG-7** — "no multi-exit relaxation… occupied single flight refuses the signal atomically; it is not reused or widened" (`:273-275`), restated with refusal code `SINGLE_FLIGHT_CONFLICT` at `docs/spec/signal-seat/05-conversion.md:37-41`. Per the `CLAUDE.md` conflict rule (accepted ADR is the target; this file is self-declared "advisory analysis… not an ADR/spec amendment", §Status) the ADR governs and **no operator decision is outstanding**. R7 sell-direction conversion is unblocked and must implement D-SIG-7/D-SIG-8 as written. Closed by the planning seat; recorded in `work/queue/SIGNAL-SEAT-R5b-TO-D2a-SEQUENCING-PLAN.md` §Decisions. |
