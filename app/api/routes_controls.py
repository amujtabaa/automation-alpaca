...
**Spine v2 Phase 6 facade migration (ADR-005):** all three commands call
`ExecutionCommandFacade` — the route no longer imports `app.store`. The
kill-switch deferral is resolved: wave 3d already made `set_kill_switch`
event_truth (the three-state `TradingState` FSM), so wrapping it behind the
facade is a pure boundary move, not a behavior change. The command's `actor`
comes from the minimal actor-audit header (`get_actor`).
"""
...
async def kill_switch(
    ...
    try:
        return await command_facade.set_kill_switch(engaged=body.engaged, actor=actor)
...
async def pause_buys(
    ...
    try:
        return await command_facade.pause_buys(actor=actor)
    except FacadeError as exc:
        raise facade_error_to_http(exc) from exc
...
async def resume_buys(
    ...
    try:
        return await command_facade.resume_buys(actor=actor)
    except FacadeError as exc:
        raise facade_error_to_http(exc) from exc
