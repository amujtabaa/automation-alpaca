...
- [ ] The **correlation contract is specified** (Codex PR #5 round-5 P2): `SIGNAL_APPROVED` ↔ created intent/candidate id, and the intent origin's `(producer_id, signal_id)` back-reference — the schema fields, event payloads, and the filter path an auditor uses to walk order → signal.
- [ ] The **approval payload is specified** (Codex PR #5 round-4 P1): approval carries operator-confirmed quantity + limit price (server-validated); producer-suggested sizing is display-only and structurally never flows into an order field — the as-built candidate path binds `suggested_quantity`/`suggested_limit_price`, so the spec must define where operator values enter.
