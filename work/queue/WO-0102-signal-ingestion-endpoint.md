...
- [ ] **Fake-rails test seam so WO-0102's own route tests are runnable in isolation** (REV-0024-F P1): the guard checks that a rails provider *satisfying the Protocol* is present, NOT that WO-0104 is merged. WO-0102 ships a **test-double rails provider** (a permissive in-memory conforming implementation — never a production default) that its own accept/dedupe/malformed/auth integration tests inject to construct a mounted flag-on app WITHOUT bypassing the guard or implementing WO-0104 early. The **paced-flood constant-event-row** and **full route-authorization matrix** tests still run at the joint enablement milestone against WO-0104's *real* rails (`03-rails.md §1a`) — the fake seam exercises ingest wiring, not the flood bound.
