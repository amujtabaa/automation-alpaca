...
- [ ] **Backend-owned launch path — the enforceable proxy-private bind** (ADR-009 A-1 clause 6, REV-0024-F-001): an in-app setting check cannot observe the real listener bind (`uvicorn app.main:app --host 0.0.0.0` sets it outside the ASGI app). Ship `app/server.py::run()` (invoked `python -m app`) that starts uvicorn **programmatically** with the bind derived from + re-validated against `signal_transport_policy`, exiting non-zero **before serving** on any non-loopback/non-socket bind; set an `app.state` launch-provenance sentinel enforced at **BOTH lifespan startup AND request time**. The request-time enforcement is a **fail-closed ASGI guard** that 503s every request when the sentinel is absent — because `uvicorn app.main:app --host 0.0.0.0 --lifespan off` **skips lifespan** and would dodge a startup-only guard (REV-0024-F P1); the request guard runs regardless of `--lifespan`. Deprecate the direct `uvicorn app.main:app` path (non-functional under the flag); document `python -m app` in README. **Test: a subprocess** that (a) launches via `app/server.py` with a non-loopback bind + flag on → pre-serve exit; (b) runs `uvicorn app.main:app --host 0.0.0.0` + flag on **with `--lifespan on` (startup fails) and with `--lifespan off` (starts, but every request 503)** — nothing reachable on the network bind either way. An app-setting-only or lifespan-only assertion does NOT satisfy this. Flag off ⇒ bare uvicorn keeps working unchanged.
...
- [ ] **Rails-presence startup guard** (ADR-009 A-4; REV-0024-F-004): with `signal_seat_enabled` on, `create_app` startup **fails fast unless a conforming rails provider is wired** (rate bucket + non-refilling invalid/conflict budget + quarantine epoch + human release) — exactly parallel to the credential-presence guard. In production that provider is WO-0104's, so this WO ships the endpoint with the flag **structurally un-enable-able against the real wiring** until WO-0104 lands and lifts the gate.
- [ ] **Sanctioned test seam so WO-0102's own route tests are runnable in isolation** (REV-0024-F P1): the flag-on app has two guards to satisfy — rails-presence (needs a conforming rails provider) and launch-provenance (needs the sentinel). WO-0102 ships (i) a **test-double rails provider** (a permissive in-memory conforming implementation — never a production default) and (ii) a **sanctioned test fixture that sets the launch-provenance sentinel through the same `app.state` seam the real launcher uses** (NOT an ad-hoc bypass that weakens the guard). Its accept/dedupe/malformed/auth integration tests use both to construct a mounted flag-on `TestClient` app without bypassing either guard or implementing WO-0104 early. The **paced-flood constant-event-row** and **full route-authorization matrix** tests still run at the joint enablement milestone against WO-0104's *real* rails (`03-rails.md §1a`) — the seam exercises ingest wiring, not the flood bound.
