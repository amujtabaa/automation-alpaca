...
- [ ] **Ingest processing order per A-4**: authenticate → rails → bounded body read (64 KiB cap) → parse; steps 1–2 reject with zero store writes and zero body processing; epoch-bounded audit (one PRODUCER_QUARANTINED per epoch, saturating out-of-log counter, count on PRODUCER_RELEASED); model-based flood test asserts constant event-row count.
