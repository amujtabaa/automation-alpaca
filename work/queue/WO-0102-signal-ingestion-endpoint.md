...
allowed_paths:
  ...
  - app/main.py                      # router mount + app construction: docs-route disable/operator-gate, transport bind-policy + credential-presence startup guards (A-1) — Codex rev-3
...
- [ ] **Operator credential required on EVERY sensitive route — reads included — from this WO onward** (ADR-009 Amendment A-1): the full route-authorization matrix ({none, invalid, producer-key, operator-key} × every mounted sensitive route incl. positions/orders/sessions/signals-list/producers) asserted against the real mounted app; transport-policy startup guard (proxy-private bind fail-fast under BOTH policies), **credential-presence startup guard** (flag on ⇒ startup fails unless OPERATOR_API_KEY + producer map configured), FastAPI auto-docs routes disabled-or-operator-only and classified, and key-lifecycle rules implemented as specified (Codex rev-3).
- [ ] **Ingest processing order per A-4**: authenticate → rails → bounded body read (64 KiB cap) → parse. **The handler takes raw `Request` — no Pydantic body parameter** (FastAPI reads body-model routes before dependencies can reject; Codex rev-2); auth/rails run as body-blind dependencies, then manual capped read + validation. Steps 1–2 reject with zero store writes and zero body processing. **This WO's interim ceiling is audit-free** — over-ceiling rejects append ZERO events; only an out-of-log saturating counter moves. The `PRODUCER_QUARANTINED`/`PRODUCER_RELEASED` epoch machinery (and its one-append-per-epoch carve-out) is **WO-0104's**, not this WO's (Codex rev-3). Model-based flood test asserts a constant (zero) signal-event append under sustained over-ceiling ingest.
...
- [ ] **Interim ingest ceiling ships WITH the endpoint** (Codex PR #5 round-6 P2): a conservative hard per-producer + global requests-per-window ceiling, boundary-rejected (429) beyond it, **audit-free — zero events, an out-of-log saturating counter only** (no `PRODUCER_QUARANTINED` epoch here; that's WO-0104) — so there is NO window between this WO and WO-0104 in which an authenticated producer can flood the append-only log. Test: sustained over-ceiling ingest appends ZERO events. WO-0104's full rails (policy limits + producer quarantine + human release) supersede this ceiling; the ceiling is not removed until they land.
