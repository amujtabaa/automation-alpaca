...
> **Enablement is the joint WO-0102 + WO-0104 milestone** (ADR-009 A-4): this WO's flag is structurally un-enable-able until WO-0104's rails satisfy the rails-presence startup guard, so the flag-on integration tests (route matrix, paced-flood) run at that milestone, not in isolation.
> Sequencing: 0101 → 0102 (endpoint, flag gated off) → 0104 (rails, lifts the gate) → 0103; 0102+0104 co-gate live enablement.
