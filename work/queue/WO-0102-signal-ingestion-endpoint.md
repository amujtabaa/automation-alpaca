...
- [x] **Schema/DB-migration human approval RECORDED before any store-schema change** (human-gated surface; `06-invariants.md §Cross-cutting` (b) + CLAUDE.md safety core): the `SignalRecord` table/index (and the additive nullable `signal_*` columns) is **additive-only, NULL-default, no rewrite of existing rows, both stores**, and its migration plan is **stopped for explicit human approval before execution** — the WO cannot close without that approval recorded. Likewise the `ExecutionEventType` additions: if the implementer judges they cross the event-log-truth human-gated surface, **escalate, do not self-decide** (Notes below). **APPROVAL RECORDED 2026-07-14 (Ameen, session confirmation):** the `SignalRecord` schema migration (durable `signal_records` table + index; additive nullable `signal_*` columns; additive-only, NULL-default, no row rewrite, both stores) and the parallel `SIGNAL_*`/`PRODUCER_*` `ExecutionEventType` vocabulary additions are approved. Ledger: `WO-0102-SCHEMA-APPROVAL`. This records the schema-migration gate only — the **Independent CODE review gate below stays OPEN**; the WO remains active.
