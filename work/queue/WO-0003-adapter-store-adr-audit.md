---
type: Work Order
title: Adapter + Store layer ADR audit
status: ready
...
# Work Order: Adapter + Store layer ADR audit

## Goal

Audit adapter/ (alpaca-py isolation) and store/ (event log, snapshots, projections, parity verifier) against its governing ADRs and invariants; report CONFIRMED / DRIFTED / SUPERSEDED per ADR clause, plus candidate new ADRs, with code evidence.
...
write_allowed:
  - work/active/WO-0003*/**   # findings report only
```
...
- [ ] Audit focus areas: alpaca-py imported ONLY in adapter, deterministic client_order_id generation, event-log append-only truth, fill dedup, projection correctness, parity verifier status.
...
- [ ] Findings report written under work/active/WO-0003*/; no source/test/ADR edits.
