...
# Work Order: UI layer ADR audit

## Goal

Audit ui/ (Streamlit thin cockpit) against its governing ADRs and invariants; report CONFIRMED / DRIFTED / SUPERSEDED per ADR clause, plus candidate new ADRs, with code evidence.
...
write_allowed:
  - work/active/WO-0005*/**   # findings report only
```
...
- [ ] Audit focus areas: UI imports only the typed API client, owns no order/fill/position state, never imports alpaca-py or engine/store, issues intents only.
...
- [ ] Findings report written under work/active/WO-0005*/; no source/test/ADR edits.
