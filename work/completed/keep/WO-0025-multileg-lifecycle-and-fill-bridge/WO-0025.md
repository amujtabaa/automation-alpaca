...
wave: W3 remediation (REV-0023 Phase A; blocks ADR-010 acceptance recommendation)
model_tier: strong
risk: high
disposition: [RESULT_SUMMARY_KEPT]
record_reconciliation: "WO-0120 (2026-07-20) verified fable-done.md status VERIFIED and the existing WO-0025 DISPOSED ledger row; a canonical CLOSED ledger row is appended."
owner: Ameen
created: 2026-07-12
---
...
## Allowed paths
```yaml
allowed_paths: [app/sellside/policy.py, app/store/core.py, app/store/memory.py, app/store/sqlite.py, app/monitoring.py, tests/**, docs/adr/ADR-010-execution-envelope.md, docs/INVARIANTS.md]
```
...
- [ ] Inferred-fill repro pinned strict: after a synthetic fill, envelope remaining decrements
      ...
      decrement total).
- [ ] ADR-010 amendment (working-order predicate + inferred-fill provenance) recorded; INV-082
      wording updated.
- [ ] Full gate green.
