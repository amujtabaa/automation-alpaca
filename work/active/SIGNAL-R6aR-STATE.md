...
| 15 | WO-0141 semantic half: proof/occupancy separation (ADR-016) | **VERIFIED (semantic half only — WO-0141 is NOT complete)** — D-1-a attribution, D-2-b valid-only contribution, `next_mintable` occupancy avoidance, D-3-c write cap, ADR-016, spec/pkl amendments, and the F-1 append-seam gate landed at `c20ca47`. Battery **4,796 passed / 11 skipped / 1 xfailed / 0 failed, branch coverage 93.14%** (floor 93.0, read from the coverage line). ruff/format/mypy(77)/lint-imports(6)/oracle(61) green. Four mutants RED→GREEN with no residue. **Still owed by this WO (list corrected 2026-07-29 — the earlier version named only two of five):** (1) the typed `ProducerRailFact` union / decode (§2 item 1); (2) one `ProducerRailMachine` with strict and tolerant policies over a single transition kernel (§2 item 5); (3) the Hypothesis `RuleBasedStateMachine` over open/close/heal/restart/append-malformed (§5.2); (4) the ADR-015 mutation baseline with `MAX_SURVIVORS` lowered from the 999 sentinel (§5.6); (5) reviewer-OWNED holdouts — what exists is pre-registered by the implementing seat, which is a weaker property and is labelled as such (§5.1). Open defect **P0-8** is routed to WO-0142. **Gate state unchanged: REV-0045 Codex-owned, P0-2/3/4/6 not independently confirmed closed, R-1/R-2 open, D-2a OFF, R6b blocked.** |
...
| 22 | The new gate took CI red on 3.11 — inside the gate built to prevent that | **RESOLVED, and recorded rather than quietly amended.** `1ecfdc0` shipped `tests/test_min_python_syntax_gate.py` and the **3.11 CI leg went red on that file** (run 30502139527; `test (3.12)` success, `test (3.11)` failure, 1 failed / 4,865 passed). **The failing assertion was mine and it was the gate's own self-check:** a bare `compile(_PLANTED_312_ONLY_SOURCE, ...)` with a comment asserting "this runtime is 3.12+". That is precisely the assumption the gate exists to eliminate, made *inside* the gate — legal on 3.12, `SyntaxError` on 3.11. **And the verification I claimed at commit time was worthless in a specific, instructive way:** "the new module AST-parses under python3.11" was TRUE — the illegal form is assembled at runtime so the file always parses — and it never executed the assertion. **Parsing a file is not running its tests.** Second time in one session that "verified under 3.12 only" shipped a 3.11 break. **Repair is stronger than the original, not merely patched:** the assertion is now version-conditional, so on 3.11 it *actively proves* the construct is refused there. **Evidence, both interpreters, executed not parsed:** built a real 3.11 venv from `requirements.txt -c constraints.txt` (Python 3.11.15, ruff 0.15.20). Gate **6/6 on 3.11 and 6/6 on 3.12**. RED/GREEN proof by reverting to the shipped form: **FAILS on 3.11, PASSES on 3.12** — exactly why it shipped — and green on both once restored. **A second finding from the same failure:** the collector keeps only strings that parse on the RUNNING interpreter, so on 3.11 a 3.12-only fixture is dropped as prose and this gate is close to vacuous there. That is correct division of labour — on 3.11 the defect is caught natively by whatever test parses the fixture — but it means "the gate passed" on 3.11 carries much less than on 3.12. Now stated in the module docstring, because undocumented it is a false sense of security. Standing rule added to `pkl/architecture/testing-model.md`: **when a check reasons about a version, platform, or store, it is not verified until it has RUN under each one** — the dual-store rule, one layer out. A local minimum-interpreter venv is cheap and removes the excuse. **CLOSED at `586b675`: CI green on BOTH legs** (run 30503366455 — `test (3.11)` success, `test (3.12)` success, every step), read from the Actions API. Full battery **4,866 passed / 11 skipped / 2 xfailed / 0 failed** on both interpreters — 93.11% on 3.12, 93.10% on 3.11. |
...
- 2026-07-30 (slice 22 — the gate against 3.12-only syntax shipped a 3.12-only assumption):

  `1ecfdc0` took the **3.11 CI leg red**, and the failing line was inside
  `tests/test_min_python_syntax_gate.py` — the gate I had just built to make exactly this class
  impossible. One test, one assertion:
  ...
  The comment states the defect out loud. "This runtime is 3.12+" is the assumption the whole gate
  exists to eliminate, and I wrote it into the gate's own self-check.

  **The part worth keeping is why my verification missed it.** The commit message says "the new module
  AST-parses under python3.11", and that was *true*: the illegal construct is assembled at runtime
  precisely so no literal in the file is illegal, which means the file parses on every interpreter —
  and parsing it never runs the assertion. I checked the property that was easy to check, found it
  green, and reported it as though it covered the property that mattered. **Parsing a file is not
  running its tests.** This is the second time in one session that "verified under 3.12 only" shipped a
  3.11 break, and the first time the excuse — no 3.11 environment — was available. It no longer is:
  a 3.11 venv built from `requirements.txt -c constraints.txt` took one command.

  **The repair is stronger than what it replaces**, which is the standard a fix on a gate should meet.
  The assertion is now version-conditional, so on 3.11 it does not merely avoid failing — it *proves*
  the construct is refused there:

  ```
  Python 3.11.15, ruff 0.15.20 (from constraints.txt)
    gate on 3.11: 6/6 passed          gate on 3.12: 6/6 passed
  RED/GREEN, reverting to the shipped form:
  ...
  **A second, subtler finding fell out of the same failure.** The collector keeps only strings that
  parse *on the running interpreter*. So on 3.11 a 3.12-only fixture does not parse, is dropped as
  prose, and this gate says nothing — it is close to vacuous on that leg. That is the right division of
  labour (on 3.11 the defect is caught natively by whichever test feeds the fixture to `ast.parse`),
  but it means a green 3.11 run of this gate carries far less than a green 3.12 run. Undocumented, that
  is a false sense of security, so it is now stated in the module docstring. Neither leg alone is the
  control.
