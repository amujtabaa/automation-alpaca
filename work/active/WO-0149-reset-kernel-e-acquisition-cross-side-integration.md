...
- [x] AC-01 (FR-01): Exact dual-mandate authority, field-level matching, and structural-only
  admission are specified and independently reviewed.
- [x] AC-02 (FR-02/03): The sealed acquisition lifecycle, exposure-increasing BUY refusal,
  target-derived BUY-cancel preservation, and create-claim revalidation routes are specified with
  failure-capable controls.
- [x] AC-03 (FR-03/04): Bounded dual-mandate effect/projection binding and one atomic first-fill
  integration transition are specified; no net-position or caller-byte substitute is accepted.
- [x] AC-04 (FR-05/06): Current-index cross-side preemption and exact `CLOSED` wait release are
  specified without private/audit scans or policy promotion.
- [x] AC-05: M1-M4 war-game has no unaddressed `ASSUMED` decision claim, and independent
