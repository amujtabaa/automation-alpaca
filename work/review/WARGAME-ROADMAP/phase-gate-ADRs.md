...
| F3 | A **maximum daily loss control exists and is enforced**, or its absence is an explicitly ratified operator acceptance recorded in this ADR. *Today no P&L control exists anywhere in `app/` — verified: `daily_loss`/`max_daily`/`drawdown`/`daily_pnl` return zero hits* | the capital-limit coverage checker |
