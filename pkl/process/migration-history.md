...
- 2026-07-11: **Pre-beta-reliance gates on the flip are now CLEARED** — ADR-008 **Accepted**
  ...
  resolved by ADR-008 clarification). Remaining known deferral: `filled_quantity` event-sourcing
  (status-only flip). `last_verified` refreshed against those documents.
- 2026-07-08: **WO-0007b — the read-flip is DONE (human sign-off given).** The order-status graph was completed in the log (`SUBMIT_RELEASED` release edge + `CANCEL_PENDING` entry — Stage A) so a latest-event-wins projector (`app/events/projectors.py::project_order_status`) can reconstruct every live state; `get_order`/`list_orders` in both stores now derive **status** from that projection (proven: a hand-corrupted `orders.status` column does not surface), with an init backfill reconstructing pre-eventing orders. The matrix's last `legacy_truth` flow ("Atomic submit claim") is now **`event_truth`** — the migration is substantially terminal. Caveats: (1) **STATUS only** — `filled_quantity` stays column-sourced (it is not universally the FILL-event sum; event-sourcing it is a separate follow-up). (2) **Pre-beta-reliance gates still open (NOT done here):** ADR-008 formal acceptance and independent cross-model review (CLAUDE.md Review policy) — the flip must not be RELIED UPON for a beta milestone until both clear. Evidence: `work/completed/keep/WO-0007b-*/` (once closed), full suite green, adversarial-verify pass.
