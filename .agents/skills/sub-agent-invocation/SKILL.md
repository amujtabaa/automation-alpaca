...
- Session files: "Read .Codex/tasks/session-current.md for full context"
...
1. Run the workflow-worthy checklist in `.Codex/commands/team-plan.md`. If two or more signals fire (massively parallel fan-out, adversarial/verification-heavy, unknown-size discovery, large-scale sort/rank, high cross-context-contamination risk, "do not stop until X"), choose `/workflow-build`.
...
- `/workflow-build`: Data-dependent or adversarial shape a fixed agent graph cannot express; a deterministic JavaScript harness orchestrates one agent per unit, plus N skeptics/judges (high, data-dependent tokens; the Workflow tool is on by default on paid plans, Codex v2.1.154+)
...
| **Orchestration author** | The lead (Codex) coordinates turn by turn                                  | The lead defines a contract chain; teammates self-coordinate                                               | A JavaScript file (`agent()` / `pipeline()` / `parallel()`) is the orchestrator                                                                                                          |
...
```
Skill({ skill: "build", args: ".Codex/tasks/<plan-file>.md" })
```

For collaborative Agent Teams execution (when agents need peer-to-peer coordination):

```
Skill({ skill: "team-build", args: ".Codex/tasks/<plan-file>.md" })
```
...
Skill({
  skill: "team-plan",
  args: "Build a new models section for the blog with 11 model profile pages. User wants: chronological model pages from Codex 3 through Opus 4.6, a models index page, and cross-links to existing posts. Existing blog structure is in apps/web/src/content/blog/blog-structure.ts. Content style should match existing guide posts. Use content-writer agents for parallel page creation."
})
```
...
- **Durable transcript**: every sub-agent run is written to `~/.Codex/projects/<project>/<session>/agent-<id>.jsonl` and kept for `cleanupPeriodDays` (default 30 days; raise it if you rely on rehydration).
