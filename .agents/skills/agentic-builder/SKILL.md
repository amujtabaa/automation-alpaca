...
description: Create and iteratively improve Codex skills, sub-agents, and slash commands. Use when users want to create a skill from scratch, build a new sub-agent, create a slash command, update or optimize an existing skill/agent/command, run evals to test a skill, benchmark skill performance, or optimize a skill's description for better triggering accuracy. Covers all three Codex primitives: skills (SKILL.md with eval loops), agents (sub-agent .md files with frontmatter), and commands (slash command .md files with $ARGUMENTS). Use this proactively whenever someone says "make a skill", "create an agent", "build a sub-agent", "make a command", "create a slash command", "turn this into a skill", or "I want to automate X".
---

# Agentic Builder

Build skills, agents, and commands for Codex. Three modes, one entry point.
...
- They want something Codex auto-delegates to
- They need a focused specialist with limited tools
- The output is a single `.md` file in `.Codex/agents/`

Agents are specialized sub-processes. Codex delegates tasks to them based on their description. Each agent has its own system prompt, tool access, and model selection.
...
- The output is a `.md` file in `.Codex/commands/`
...
| --------------------- | ------------------------------------------------ | ----------------------------------------- | ----------------------------------------------- |
| **File**              | `SKILL.md` in a folder                           | Single `.md` in `.Codex/agents/`         | Single `.md` in `.Codex/commands/`             |
...
- If the task is a focused specialist that Codex should delegate to -> **agent**
